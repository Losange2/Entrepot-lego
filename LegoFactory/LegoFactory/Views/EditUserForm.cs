...
namespace LegoFactory
{
    public class EditUserForm : Form
    {
        ...
        public string UserLogin => tbLogin.Text.Trim();
        public string Password => tbPassword.Text;
        public string SelectedRole => cbRole.SelectedItem?.ToString() ?? "Employe";

        public EditUserForm(string nom, string login, string role)
        {
            Text = $"✏️ Modifier l'utilisateur - {login}";
            ...
            tbNom = new TextBox { Location = new Point(160, 22), Width = 240, Font = new Font("Segoe UI", 10F), Text = nom };
            ...
            tbLogin = new TextBox { Location = new Point(160, 62), Width = 240, Font = new Font("Segoe UI", 10F), Text = login };
            ...
            tbPassword = new TextBox { Location = new Point(160, 102), Width = 240, Font = new Font("Segoe UI", 10F), PasswordChar = '*', PlaceholderText = "(laisser vide = inchangé)" };
            ...
            btnOk.Click += (s, e) =>
            {
                if (string.IsNullOrWhiteSpace(tbNom.Text) || string.IsNullOrWhiteSpace(tbLogin.Text))
                {
                    MessageBox.Show("Le nom et le login sont obligatoires.", "Champs manquants", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
